...
class RBreaker(BasePolicy):
    ...
    def run(self, api, symbol):
        ...
        SYMBOL = symbol
        CLOSE_HOUR, CLOSE_MINUTE = 1, 50  # 平仓时间
        STOP_LOSS_PRICE = 5  # 止损点(价格)
        ...
        while True:
            ...
            if api.is_changing(quote, "datetime"):
                ...
                if now.hour == CLOSE_HOUR and now.minute >= CLOSE_MINUTE:  # 到达平仓时间: 平仓
                    ...
                    print("临近本交易日收盘: 平仓", account.balance)
                    target_pos_value = 0  # 平仓
                    pivot = bBreak = sSetup = sEnter = bEnter = bSetup = sBreak = float(
                        "nan")  # 修改各指标线的值, 避免平仓后再次触发
            ...
            if api.is_changing(quote, "last_price"):
                ...
                if target_pos_value > 0:  # 多头持仓
                    if quote.highest > sSetup and quote.last_price < sEnter:
                        ...
                        target_pos_value = -pos_change  # 做空
                    ...
                    if quote.lowest < bSetup and quote.last_price > bEnter:
                        ...
                        target_pos_value = pos_change  # 做多
                        open_position_price = quote.last_price
