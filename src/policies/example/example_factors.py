...
def pearson_corr(df_, target):
    ...
    for i in df.columns.values:
        if (
            ...
            Pearson_dict[i] = scipy.stats.pearsonr(
                df[target].values, df[i].values)[0]
...
def series_sum(S, N):  # 对序列求N天累计和，返回序列    N=0对序列所有依次求和
    return (
        pd.Series(S).rolling(N).sum().values if N > 0 else pd.Series(
            S).cumsum().values
    )
...
class generalFactors:
    ...
    def ktn(df, KTN_type, N=20, M=10):  # 肯特纳交易通道, N选20日，ATR选10日
        MID = ema((df["high_price"] + df["low_price"] +
                  df["close_price"]) / 3, N)
        if KTN_type == "KTN_mid":
            return MID
    ...
    def brar(df, M1=26):  # BRAR-ARBR 情绪指标
        ...
        return (
            series_sum(np.maximum(
                0, df["high_price"] - ref(df["close_price"], 1)), M1)
