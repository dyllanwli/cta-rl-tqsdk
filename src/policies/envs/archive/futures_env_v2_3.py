    ...
    def _get_state(self):
        ...
        while True:
            ...
            if self.api.is_changing(self.bar_1s.iloc[-1], "datetime"):
                ...
                # bar_1m = self.bar_1m[self.OHLCV].iloc[-self.data_length[Interval.ONE_MIN.value]:].to_numpy(dtype=np.float32)
                # bar_30m = self.bar_30m[self.OHLCV].iloc[-self.data_length[Interval.THIRTY_MIN.value]:].to_numpy(dtype=np.float32)
