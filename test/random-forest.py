...
api = TqApi(backtest=TqBacktest(start_dt=datetime.date(
    2018, 7, 2), end_dt=datetime.date(2018, 9, 26)))
...
with closing(api):
    try:
            ...
            while True:
                ...
                if api.is_changing(quote, "datetime"):
                    ...
                    if now.hour == close_hour and now.minute >= close_minute:
                        # 1- 获取数据
                        x_train, y_train, x_predict = get_prediction_data(
                            klines, 75)  # 参数1: K线, 参数2:需要的数据长度
        ...
        # 增加预测的本交易日涨跌情况字段(向后移一个数据目的: 将 本交易日对应下一交易日的涨跌 调整为 本交易日对应本交易日的涨跌)
